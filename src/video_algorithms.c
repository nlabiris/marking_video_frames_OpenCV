...
void absdiff(IplImage *previous_frame, IplImage *current_frame, IplImage *new_frame, int frame, FILE *fp, int width_img, int height_img, int *index_call)
{
	...
	for(i=0;i<height_img;i++)
	{
		...
		{
			poutput_new[i*new_frame->widthStep+j*new_frame->nChannels+0]=abs(pinput_current[i*current_frame->widthStep+j*current_frame->nChannels+0]-pinput_previous[i*previous_frame->widthStep+j*previous_frame->nChannels+0]); // B
			poutput_new[i*new_frame->widthStep+j*new_frame->nChannels+1]=abs(pinput_current[i*current_frame->widthStep+j*current_frame->nChannels+1]-pinput_previous[i*previous_frame->widthStep+j*previous_frame->nChannels+1]); // G
			poutput_new[i*new_frame->widthStep+j*new_frame->nChannels+2]=abs(pinput_current[i*current_frame->widthStep+j*current_frame->nChannels+2]-pinput_previous[i*previous_frame->widthStep+j*previous_frame->nChannels+2]); // R
		}
	}
	...
	for(i=0;i<height_img;i++)
	{
		for(j=0;j<width_img;j++)
		{
			sum0=sum0+poutput_new[i*new_frame->widthStep+j*new_frame->nChannels+0]; // B
			sum1=sum1+poutput_new[i*new_frame->widthStep+j*new_frame->nChannels+1]; // G
			sum2=sum2+poutput_new[i*new_frame->widthStep+j*new_frame->nChannels+2]; // R
		}
	}
	...
	if(similarity0 < 90 || similarity1 < 90 || similarity2 < 90)  // If similarity in any of the 3 channels (RGB) is < 95% then the frame is marked
	{
		*index_call=1;
		if(frame<10)
		{
...
{
	int encoded_diff=0;		// Compression difference between frames
	...
	// Threshold to mark frames we want
	if(encoded_diff>(unsigned int)threshold)	// If the difference in bytes between the previous and current frame is > 0.5% of the whole image (in size) then the frame is marked
	{
		*index_call=1;
		if(frame<10)
		{
...
{
	// Threshold to mark frames we want
	if(ecr_call[frame] > (*ecr_max_call)*0.3)	// if the ECR of the frame is > of the 30% of the max ECR of the whole video then the frame is marked
	{
		*index_call=1;
		if(frame<10)
		{
...
void ecrdiff(IplImage *current, IplImage *previous, CvSize size, int frame, FILE *fp, int *index_call)
{
	...
	double wx[3][3], wy[3][3];						// X-axis, Y-axis masks (Sobel)
	int w_xor[3][3];							// Mask (Dilation)
	int t_xor=0;								// Sum variables (Dilation)
	double edge_pixels_previous=0;						// Edge pixels (previous)
	double edge_pixels_current=0;						// Edge pixels (previous)
	double exiting_pixels_previous=0;					// Exiting pixels (previous)
	double exiting_pixels_current=0;					// Exiting pixels (current)
...
// ---------------------------->  MASK for Dilation
	...
	for (i=1;i<current->height-1;i++)
	{
		for (j=1;j<current->width-1;j++)
		{
			...
			for (k=0;k<3;k++)
			{
				for (l=0;l<3;l++)
				{
					tx = tx + poutput_previous_gray[(i+k)*previous_gray->widthStep+(j+l)*previous_gray->nChannels+0] * wx[k][l];
				}
			}
			...
			for (k=0;k<3;k++)
			{
				for (l=0;l<3;l++)
				{
					ty = ty + poutput_previous_gray[(i+k)*previous_gray->widthStep+(j+l)*previous_gray->nChannels+0] * wy[k][l];
				}
			}
			...
			if (t>255) t=255;
			if (t<0) t=0;
			poutput_previous_gray[i*previous_gray->widthStep+j*previous_gray->nChannels+0]=(unsigned char) t;
		}
	}
	...
	for (i=1;i<current->height-1;i++)
	{
		for (j=1;j<current->width-1;j++)
		{
			...
			for (k=0;k<3;k++)
			{
				for (l=0;l<3;l++)
				{
					tx = tx + poutput_current_gray[(i+k)*current_gray->widthStep+(j+l)*current_gray->nChannels+0] * wx[k][l];
				}
			}
			...
			for (k=0;k<3;k++)
			{
				for (l=0;l<3;l++)
				{
					ty = ty + poutput_current_gray[(i+k)*current_gray->widthStep+(j+l)*current_gray->nChannels+0] * wy[k][l];
				}
			}
			...
			if (t>255) t=255;
			if (t<0) t=0;
			poutput_current_gray[i*current_gray->widthStep+j*current_gray->nChannels+0]=(unsigned char) t;
		}
	}
	...
	for (i=1;i<current->height-1;i++)
	{
		for (j=1;j<current->width-1;j++)
		{
			...
			for (k=0;k<3;k++)
			{
				for (l=0;l<3;l++)
				{
					t_xor = t_xor + (poutput_previous_gray[(i+k)*previous_gray->widthStep+(j+l)*previous_gray->nChannels+0] ^ w_xor[k][l]);
				}
			}
			if (t_xor>255) t_xor=255;
			if (t_xor<0) t_xor=0;
			pinput_previous_gray[i*previous_gray->widthStep+j*previous_gray->nChannels+0]=(unsigned char) t_xor;
		}
	}
	...
	for (i=1;i<current->height-1;i++)
	{
		for (j=1;j<current->width-1;j++)
		{
			...
			for (k=0;k<3;k++)
			{
				for (l=0;l<3;l++)
				{
					t_xor = t_xor + (poutput_current_gray[(i+k)*current_gray->widthStep+(j+l)*current_gray->nChannels+0] ^ w_xor[k][l]);
				}
			}
			if (t_xor>255) t_xor=255;
			if (t_xor<0) t_xor=0;
			pinput_current_gray[i*current_gray->widthStep+j*current_gray->nChannels+0]=(unsigned char) t_xor;
		}
	}
	...
	// Threshold to mark frames we want
	if(ecrdiff > 0.5)	// if the ECR of the frame is > of the 30% of the max ECR of the whole video then the frame is marked
	{
		*index_call=1;
		if(frame<10)
		{
